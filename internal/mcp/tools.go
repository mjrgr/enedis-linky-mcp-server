...
func (s *Server) registerGetDailyConsumption() {
	tool := mcp.NewTool("get_daily_consumption",
		...
		mcp.WithString("prm",
			mcp.Description("PRM (Point de Référence de Mesure) — the meter's unique 14-digit identifier. Optional if LINKY_PRM env var is set."),
		),
	...
	s.mcp.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		prm, start, end, err := extractDateRangeParams(req, s.prm)
		if err != nil {
...
// defaultPRM is used as fallback when the caller omits the prm parameter.
func extractDateRangeParams(req mcp.CallToolRequest, defaultPRM string) (prm, start, end string, err error) {
	prm = req.GetString("prm", defaultPRM)
	if prm == "" {
		return "", "", "", fmt.Errorf("parameter 'prm' is required and must be a non-empty string (or set LINKY_PRM env var)")
	}
